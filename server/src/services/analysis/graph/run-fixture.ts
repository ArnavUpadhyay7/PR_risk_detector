  ...
  return (systemPrompt: string, _userPrompt: string): unknown => {
        ...
        findings: [
          {
            ...
            description:
              "authMiddleware assigns req.user from jwt.verify without validating required claims exist before downstream route handlers read them.",
            file: "src/middleware/auth.ts",
            recommendation:
              "Validate decoded token shape and required claims before attaching req.user.",
          },
        ],
      });
    }
        ...
        findings: [
          {
            ...
            description:
              "Admin route now checks req.user.isAdmin, but no test changes were included to verify forbidden access for non-admin users.",
            file: "src/routes/admin.ts",
            recommendation:
              "Add authorization tests covering admin-only access and forbidden responses.",
          },
          {
            severity: "CRITICAL",
            title: "JWT secret reliance without rotation strategy",
            description:
              "Login and middleware rely on process.env.JWT_SECRET with no mention of key rotation or invalidation for expired tokens.",
            file: "src/routes/login.ts",
            recommendation:
              "Document secret management and add tests for expired or missing tokens.",
          },
        ],
      });
    }
        ...
        findings: [
          {
            ...
            description:
              "Session-based login was replaced with JWT issuance, but no test files were modified in this PR.",
            recommendation:
              "Add tests for login token issuance, missing token responses, and invalid token handling.",
          },
        ],
      });
    }
    ...
    if (systemPrompt.includes("final merge-risk judge")) {
      return riskJudgeOutputSchema.parse({
        ...
        riskScore: 72,
        summary:
          "This PR introduces JWT authentication and admin authorization checks without test coverage. Missing token validation edge cases and absent auth tests create meaningful merge risk.",
        bugRisk: 45,
        securityRisk: 82,
        testingRisk: 78,
        recommendations: [

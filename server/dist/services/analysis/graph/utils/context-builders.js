...
const SECURITY_FILE_PATTERN = /(?:^|\/)(?:auth|authentication|security|password|credential|token|secret|oauth|jwt|session|permission|acl|rbac|encrypt|crypto|ssl|tls|middleware)(?:\/|\.|$)/i;
...
function isPerformanceFile(filename) {
    return PERF_FILE_PATTERN.test(filename) || DB_FILE_PATTERN.test(filename);
}
function buildAnnotatedDiff(fileDiffs, maxChars) {
    const sections = [];
    let totalChars = 0;
    for (const fileDiff of fileDiffs) {
        ...
        const body = fileDiff.lines
            ...
            const location = line.newLine !== undefined
                ? `[new:${line.newLine}]`
...
function selectFiles(files, matcher, limit) {
...
function buildContext(title, description, files, fileDiffs, extraLines = []) {
...
export function toFileDiffResponse(fileDiffs) {
    return fileDiffs.map((fileDiff) => ({
        ...
        lines: fileDiff.lines.map((line) => {
            ...
            if (line.oldLine !== undefined)
                mapped.oldLine = line.oldLine;
            if (line.newLine !== undefined)
                mapped.newLine = line.newLine;
            return mapped;
        }),
    }));
}

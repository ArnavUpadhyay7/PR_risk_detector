...
import { evidenceMatchesDiff, getDiffPositionForLine, lineIsInChangedHunk, } from "./parsePatch.js";
function normalizeFilename(filename, fileDiffs) {
    if (fileDiffs.some((diff) => diff.filename === filename)) {
        return filename;
    }
    const suffixMatch = fileDiffs.find((diff) => filename.endsWith(diff.filename) || diff.filename.endsWith(filename));
    return suffixMatch?.filename ?? null;
}
export function validateAndEnrichFinding(raw, category, fileDiffs) {
...
export function validateFindings(rawFindings, category, fileDiffs) {
    const validated = [];
...
//# sourceMappingURL=validateFindings.js.map
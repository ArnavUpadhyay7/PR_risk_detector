...
function findingsSimilar(left, right) {
...
function mergeFindings(primary, secondary) {
    return {
        ...primary,
        severity: severityWeight(secondary.severity) > severityWeight(primary.severity)
            ? secondary.severity
            : primary.severity,
        ...
        recommendation: primary.recommendation.length >= secondary.recommendation.length
...
export function dedupeFindings(findings) {
    ...
    for (const finding of findings) {
        const existingIndex = deduped.findIndex((candidate) => findingsSimilar(candidate, finding));
        if (existingIndex === -1) {
    ...
    return deduped.sort((left, right) => severityWeight(right.severity) - severityWeight(left.severity));
}
...
export function computeOverallRisk(findings, scores) {
...
//# sourceMappingURL=dedupeFindings.js.map
const TEST_FILE_PATTERN = /(?:^|\/)(?:__tests__|test|tests|spec|__spec__)(?:\/|$)|\.(?:test|spec)\.[a-z0-9]+$/i;
const SECURITY_FILE_PATTERN = /(?:^|\/)(?:auth|authentication|security|password|credential|token|secret|oauth|jwt|session|permission|acl|rbac|encrypt|crypto|ssl|tls)(?:\/|\.|$)/i;
const AUTH_KEYWORD_PATTERN = /\b(auth|authentication|authorization|jwt|token|session|password|credential|oauth|permission|rbac)\b/i;
...
const PERF_KEYWORD_PATTERN = /\b(forEach|map\(|reduce\(|await\s+.+\(|query\(|findAll|findMany|loop|cache|render|sync)\b/i;
...
function isDocOnlyFile(filename) {
    const ext = getExtension(filename);
    return ext !== null && DOC_EXTENSIONS.has(ext);
}
function patchContainsKeyword(patch, pattern) {
    return patch !== undefined && pattern.test(patch);
}
export function classifyChanges(files, summary, title, description) {
    const areas = new Set();
    let codeFileCount = 0;
    ...
    const securityRelevant = summary.securitySensitive || hasAuthSignal || hasApiSignal || hasConfigSignal;
    const qualityRelevant = hasImplementationChanges && !docsOnly;
    const performanceRelevant = hasPerfSignal || hasDbSignal || (hasImplementationChanges && areas.has("backend"));
    if (docsOnly) {
        areas.add("UI");
    }
    return {
...
export function countRelevantAgents(classification) {
    let count = 0;
...
export function isTrivialChange(classification) {
    return (!classification.bugRelevant &&

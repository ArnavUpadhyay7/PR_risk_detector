...
import { buildAggregatorContext, toFileDiffResponse, } from "../utils/context-builders.js";
import { computeOverallRisk, dedupeFindings, scoreCategory, } from "../utils/diff/dedupeFindings.js";
...
export async function riskAggregatorNode(state) {
    ...
    let recommendations = findings.length > 0
        ? findings.slice(0, 4).map((finding) => finding.recommendation)
        : [];
    const warnings = [...state.agentWarnings];
    if (findings.length > 0) {
        const llm = await invokeStructuredSafe(aggregatorOutputSchema, SYSTEM_PROMPT, buildAggregatorContext({
            title: state.title,
    ...
    if (!state.deterministicSummary.testsChanged &&
...
//# sourceMappingURL=risk-aggregator.node.js.map
...
export async function runSpecialistAgent(state, config) {
    startTimer(config.timerLabel);
    console.log(`[AI] ${config.timerLabel}: started`);
    const userPrompt = config.buildContext(state);
    const result = await invokeStructuredSafe(findingsOutputSchema, `${config.systemPrompt}\n${SHARED_RULES}`, userPrompt, "specialist");
    ...
    const validated = validateFindings(result.data.findings, config.category, state.fileDiffs);
    return {
